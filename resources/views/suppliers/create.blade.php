...
@section('content')
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">{{ __('Add New Supplier') }}</h1>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
  </div>
              ...
              <form action="{{ route('suppliers.store') }}" method="POST">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control @error('name_supplier') is-invalid @enderror"
                    id="name_supplier" name="name_supplier" value="{{ old('name_supplier') }}" placeholder="Enter supplier name" required>
                  @error('name_supplier')
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control @error('address_supplier') is-invalid @enderror" id="address_supplier"
                    name="address_supplier" rows="3" placeholder="Enter supplier address" required>{{ old('address_supplier') }}</textarea>
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control @error('merchant') is-invalid @enderror" id="merchant"
                    name="merchant" value="{{ old('merchant') }}" placeholder="Enter merchant name" required>
                  @error('merchant')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control @error('sosmed') is-invalid @enderror" id="sosmed"
                    name="sosmed" value="{{ old('sosmed') }}" placeholder="Enter social media link">
                  @error('sosmed')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
